...
v0.11 — `schema_version` propagation becomes validation-based instead of blanket: on every schema update, each entry's stored data is validated against the new field definitions, and only entries compatible with the new shape are bumped to the new version (§2 R38). This replaces R21's blanket bump and makes deconfliction automatic — an entry whose conflict is resolved by a change (a required field becoming optional, the conflicted field being deleted, a type-changed field being deleted) is bumped without per-change special-casing. The R35 retarget gate is unchanged (a mixed delete+retarget PATCH skips the bump). §2 R21, R38; §7 delete-field example updated.
